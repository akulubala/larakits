    ...
    <div class="row">
        <div class="col-md-12 raw-margin-bottom-24 raw-margin-top-24">
            <div class="pull-right">
                {!! Form::open(['url' => 'admin/teams/search']) !!}
                <input class="form-control form-inline pull-right" name="search" placeholder="Search">
                {!! Form::close() !!}
            </div>
            <a class="btn btn-primary pull-left" href="{!! route('admin.teams.create') !!}">Create New</a>
        </div>
                    ...
                    <tbody>
                        @foreach($teams as $team)
                            <tr>
                                <td>{{ $team->name }}</td>
                                <td>
                                    <form method="post" action="{!! url('admin/teams/'.$team->id) !!}">
                                        {!! csrf_field() !!}
                                        {!! method_field('DELETE') !!}
                                    ...
                                    <a class="btn btn-warning pull-right btn-xs raw-margin-right-16" href="{!! route('admin.teams.edit', [$team->id]) !!}"><i class="fa fa-pencil"></i> Edit</a>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
